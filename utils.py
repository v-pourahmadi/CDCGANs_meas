...
def visualize(sess, dcgan, config, option):
  ...
  if option == 0:
    ...
    for idx in [random.randint(0, 99) for _ in xrange(100)]:
      ...
      try:
        make_gif(samples, '/output/test_gif_%s.gif' % (idx))
      except:
        save_images(samples, [image_frame_dim, image_frame_dim], '/output/test_%s.png' % strftime("%Y-%m-%d %H:%M:%S", gmtime()))
  elif option == 3:
    values = np.arange(0, 1, 1./config.batch_size)
    for idx in xrange(100):
      print(" [*] %d" % idx)
    ...
    for idx in xrange(100):
      ...
      make_gif(image_set[-1], '/output/test_gif_%s.gif' % (idx))

    new_image_set = [merge(np.array([images[idx] for images in image_set]), [10, 10]) \
        for idx in range(64) + range(63, -1, -1)]
    make_gif(new_image_set, '/output/test_gif_merged.gif', duration=8)
